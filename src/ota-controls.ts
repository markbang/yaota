...
import { admin, publisher, fail, object, strings, text } from "./ota-protocol.ts";
...
async function releaseAction(c: OtaContext, id: string, action: string, data: JsonObject) {
  const row = await getRelease(c.env, id);
  ...
  if (action === "rollout") {
    ...
    await event(c.env, "rollout", `${id}: ${amount}%`).run();
    return getRelease(c.env, id);
  }
  let source = row;
  if (action === "rollback" || action === "embedded") {
    ...
    const previous = action === "embedded" ? null : data.targetId
      ? await getRelease(c.env, text(data.targetId, "targetId"))
...
controls.post("/ota-publish/releases/:id/activate", async c => {
  publisher(c);
  const row = await getRelease(c.env, c.req.param("id"));
  ...
  const statements = [c.env.DB.prepare("UPDATE releases SET status='Live',revision=revision+1,created_at=strftime('%Y-%m-%dT%H:%M:%fZ',max(julianday('now'),(SELECT max(julianday(created_at))+0.00000002315 FROM releases))) WHERE id=? AND revision=? AND NOT EXISTS(SELECT 1 FROM releases WHERE app_id=? AND branch=? AND platform=? AND runtime_version=? AND status='Live' AND rollout<100)")
    ...
    c.env.DB.prepare("UPDATE releases SET manifest_json=json_set(manifest_json,'$.createdAt',created_at) WHERE id=?").bind(row.id), event(c.env, "activate", row.id)];
  await c.env.DB.batch(statements);
  return c.json(publicRelease(await getRelease(c.env, row.id)));
});

controls.put("/api/ota/channels/:name", async c => {
  const name = text(c.req.param("name"), "channel");
  const data = await body(c);
  const current = await c.env.DB.prepare("SELECT * FROM ota_channels WHERE app_id=? AND name=?").bind(c.env.OTA_APP_ID, name).first<ChannelRow>();
  ...
  const statement = current
    ? c.env.DB.prepare("UPDATE ota_channels SET branch=?,rollout_branch=?,percentage=?,headers_json=?,revision=revision+1 WHERE app_id=? AND name=? AND revision=?")
      .bind(branch, candidate, amount, JSON.stringify(headers), c.env.OTA_APP_ID, name, current.revision)
    : c.env.DB.prepare("INSERT INTO ota_channels(app_id,name,branch,rollout_branch,percentage,seed,headers_json) VALUES(?,?,?,?,?,?,?)")
      .bind(c.env.OTA_APP_ID, name, branch, candidate, amount, crypto.randomUUID(), JSON.stringify(headers));
  await c.env.DB.batch([statement, ...assertChanged(c.env), event(c.env, `channel-${action}`, name)]);
  return c.json(await c.env.DB.prepare("SELECT * FROM ota_channels WHERE app_id=? AND name=?").bind(c.env.OTA_APP_ID, name).first());
});
