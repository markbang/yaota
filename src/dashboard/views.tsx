...
export function Pager({ page, count, pageSize, onChange }: { page: number; count: number; pageSize: number; onChange: (page: number) => void }) {
  ...
  return <section className="data-section" aria-label="Updates">
    ...
    <Table className="data-table"><Table.ScrollContainer><Table.Content aria-label="Updates" className="releases-table"><Table.Header><Table.Column isRowHeader>Update</Table.Column><Table.Column>Branch / Platform</Table.Column><Table.Column>Bundle / Assets</Table.Column><Table.Column>Smallest patch</Table.Column><Table.Column>Status</Table.Column><Table.Column>Created</Table.Column><Table.Column aria-label="Update actions" /></Table.Header>
      <Table.Body items={filtered.slice(currentPage * size, (currentPage + 1) * size)} renderEmptyState={() => <Empty icon={<Layers size={24} />} title={releases.length ? "No matching updates" : "No updates yet"} action={releases.length ? <Button variant="secondary" size="sm" onPress={reset}>Clear filters</Button> : publishAction} />}>
        {release => <Table.Row id={release.id}><Table.Cell><div className="update-name"><span className={`update-mark ${release.directive ? "directive-mark" : ""}`}><Layers size={16} /></span><div className="cell-stack"><strong title={release.note}>{release.directive ? "Embedded rollback" : release.version || "Update"}</strong><code title={release.id}>{release.id.slice(0, 8)}</code><small className="mobile-update-context">{release.branch} / {release.platform === "ios" ? "iOS" : "Android"}</small>{release.manifest && <small className="mobile-update-context">Bundle {formatBytes(release.delivery?.bundleBytes)}</small>}{!!release.delivery?.patches.length && <small className="mobile-update-context">Patch {formatBytes(release.delivery.patches[0]!.bytes)}</small>}</div></div></Table.Cell>
          <Table.Cell><div className="cell-stack"><span className="branch-label" title={release.branch}><GitBranch size={13} /><span>{release.branch}</span></span><small>{release.platform === "ios" ? "iOS" : "Android"}</small><code className="runtime-value" title={release.runtimeVersion}>{release.runtimeVersion}</code></div></Table.Cell>
          <Table.Cell>{release.manifest ? <div className="cell-stack"><span className="mono">{formatBytes(release.delivery?.bundleBytes)}</span><small>Assets {formatBytes(release.delivery?.assetBytes)}</small></div> : <span className="muted">No bundle</span>}</Table.Cell>
          <Table.Cell>{release.delivery?.patches[0] ? <div className="cell-stack"><span className="mono">{formatBytes(release.delivery.patches[0].bytes)}</span><small className="savings-text">{release.delivery.patches[0].savingsPercent === null ? "BSDIFF" : `${release.delivery.patches[0].savingsPercent}% smaller`}</small></div> : <span className="muted">{release.manifest ? "Not recorded" : "Not applicable"}</span>}</Table.Cell><Table.Cell><div className="cell-stack"><Status status={release.status} />{release.status === "Live" && <Coverage value={release.rollout} />}</div></Table.Cell><Table.Cell><time dateTime={release.createdAt} className="table-date">{formatDate(release.createdAt)}</time></Table.Cell>
...
export function ChannelsView({ channels, onManage, createAction }: { channels: Channel[]; onManage: (channel: Channel) => void; createAction: ReactNode }) {
  return <section className="data-section"><Table className="data-table"><Table.ScrollContainer><Table.Content aria-label="Channels" className="channels-table"><Table.Header><Table.Column isRowHeader>Channel</Table.Column><Table.Column>Distribution</Table.Column><Table.Column>Candidate rollout</Table.Column><Table.Column>Server headers</Table.Column><Table.Column aria-label="Channel actions" /></Table.Header>
    <Table.Body items={channels} renderEmptyState={() => <Empty icon={<GitBranch size={24} />} title="No channels yet" action={createAction} />}>
      {channel => <Table.Row id={channel.name}><Table.Cell><div className="cell-stack"><strong>{channel.name}</strong><small>{channel.implicit ? "Default mapping" : channel.rollout_branch ? `${channel.percentage}% candidate` : "Mapped"}</small></div></Table.Cell>
